...
const OwnerResolvedTickets = ({search, activeTab}) =>{
    ...
    const loadData = async () => {
        try{
            const res = await getOwnerTickets( {id:userInfo._id, page, rowsPerPage, category, subCategory, status:"Resolved", startDate, endDate, date, search, activeTab,sortColumn, order} ).unwrap();
    ...
    },[page,rowsPerPage,category, subCategory, status, startDate, endDate, date, search, activeTab,sortColumn, order]);  //when the values that are inside the box brackets change,each time loadData function runs
...
export default OwnerResolvedTickets;